          ...
          <ul className="space-y-1">
            {microservice.next_steps.slice(0, 2).map((step, index) => (
              <li key={index} className="flex items-start gap-1.5 text-xs text-slate-600">
                <Circle size={12} className="mt-0.5 flex-shrink-0 text-slate-400" />
...
export function EmptyCard({ message }: { message: string }) {
  return (
    <div className="kanban-card border-dashed border-2 border-slate-200 bg-slate-50 text-center py-8">
